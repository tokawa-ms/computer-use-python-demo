...
def execute_action(
    *,
    ...
    if action_type == "screenshot":
        ...
        screenshot_path = image_processing.choose_model_image(screenshot_path, annotated_path)
        ...
        if not isinstance(scroll_x, int) or not isinstance(scroll_y, int):
            raise ValueError(f"Invalid scroll values: scroll_x={scroll_x} scroll_y={scroll_y}")
        ...
        screenshot_path = image_processing.choose_model_image(after_path, annotated_path)
        ...
        session.log_session_event(step=step, kind="action", detail=f"keypress keys={keys}")
        time.sleep(0.5)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for step in range(1, MAX_STEPS + 1):
        # 確認が必要な場合は自動確認を送信
        response, computer_call = confirmation.get_or_confirm_computer_call(response)
            ...
            input=[
                {
                    ...
                    "output": {
                        "type": "input_image",
                        "image_url": image_processing.image_file_to_data_url(screenshot_path),
                    },
                }
            ],
            truncation="auto",
