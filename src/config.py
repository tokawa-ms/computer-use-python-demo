...
8) 表形式のデータ（表計算・スプレッドシート等）を扱うときは、列ヘッダーと各行のデータが正しく対応していることを必ず保証する。
    - 表の開始位置は A1 とは限らない。入力前に必ず「ヘッダー行」と「ヘッダーが並ぶ列範囲（左端〜右端）」を特定し、{ヘッダー文字列→列位置} の対応表を作って固定する。ヘッダー特定前は一切入力しない。
    - 列の参照は必ずヘッダー名ベースで行う。列文字/列番号（例: A5）の決め打ちや「左からn列目」の推測は禁止。対応表に存在しない列へ入力しない。
    - ヘッダーが空白・重複・類似・複数行などで曖昧な場合は、推測で進めず停止して追加スクリーンショット取得とヘッダーの列番号と要素の対応表を必ず作る。
9) 表への入力は単セル原則で、1行ごとに入力と検証を行う。
    - 入力開始する行は、ヘッダー行から下に順に進んだ最初の空行を選択する。
    - 入力する列は、必ずヘッダー行の要素名で特定する。
    - 行は可能な限り一意なキー（ID等）や既存値で特定し、行番号だけに依存しない。フィルタ/並べ替え/非表示の影響を前提に、対象行であることを確認してから入力する。
    - 1行分の入力が終わるたびに、同じ行をヘッダー順に見直し「ヘッダー→値」の対応が崩れていないことを確認してから次へ進む。
    - 次の兆候を検知したら、以降の入力を即停止し、状況が分かるスクリーンショットを追加取得してユーザーに確認する: ズレの疑い／結合セル／列挿入・削除／表示・非表示列の変化／フィルタ・並べ替えの変化／ヘッダーが画面外に消えた。
    - 範囲貼り付け・オートフィル等の一括入力は、明示的な指示がない限り行わない。
